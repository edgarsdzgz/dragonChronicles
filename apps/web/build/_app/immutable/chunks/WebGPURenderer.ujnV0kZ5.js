...
import { g as L } from './getTextureBatchBindGroup.CknAdSLP.js';
import {
  f as de,
  S as C,
  a as T,
...
import { C as R } from './CanvasPool.6X9XNlkG.js';
const y = F.for2d();
class O {
  start(e, t, r) {
    const s = e.renderer,
  ...
  execute(e, t) {
    ...
    if (!t.bindGroup) {
      const u = t.textures;
      t.bindGroup = L(u.textures, u.count, s.limits.maxBatchableTextures);
    }
    y.blendMode = t.blendMode;
    const n = s.bindGroup.getBindGroup(t.bindGroup, r, 1),
      o = s.pipeline.getPipeline(this._geometry, r, y, t.topology);
...
O.extension = { type: [p.WebGPUPipesAdaptor], name: 'batch' };
class I {
  constructor(e) {
  ...
  _createBindGroup(e, t, r) {
    ...
    for (const l in i) {
      ...
      if (h._resourceType === 'uniformGroup') {
        ...
        const _ = d.buffer;
        f = { buffer: o.buffer.getGPUBuffer(_), offset: 0, size: _.descriptor.size };
      } else if (h._resourceType === 'buffer') {
        const d = h;
        f = { buffer: o.buffer.getGPUBuffer(d), offset: 0, size: d.descriptor.size };
      } else if (h._resourceType === 'bufferResource') {
        const d = h;
        f = { buffer: o.buffer.getGPUBuffer(d.buffer), offset: d.offset, size: d.size };
      ...
      n.push({ binding: i[l], resource: f });
    }
    const u = o.shader.getProgramData(t).bindGroups[r],
      c = s.createBindGroup({ layout: u, entries: n });
...
I.extension = { type: [p.WebGPUSystem], name: 'bindGroup' };
class W {
  constructor(e) {
  ...
  createGPUBuffer(e) {
    ...
    return (
      (e._updateID = 0),
      e.data && (de(e.data.buffer, t.getMappedRange()), t.unmap()),
...
W.extension = { type: [p.WebGPUSystem], name: 'buffer' };
class ve {
  ...
  addEmptyGroup(e) {
    ...
    let r = t + e * 4;
    if (
      ((r = Math.ceil(r / this._minUniformOffsetAlignment) * this._minUniformOffsetAlignment),
...
V.extension = { type: [p.WebGPUSystem], name: 'colorMask' };
class U {
  constructor(e) {
    this._renderer = e;
  }
  ...
  async _createDeviceAndAdaptor(e) {
    const t = await B.get().getNavigator().gpu.requestAdapter({
      ...
      r = ['texture-compression-bc', 'texture-compression-astc', 'texture-compression-etc2'].filter(
        (i) => t.features.has(i),
      ),
...
U.extension = { type: [p.WebGPUSystem], name: 'device' };
U.defaultOptions = { powerPreference: void 0, forceFallbackAdapter: !1 };
class N {
  constructor(e) {
...
N.extension = { type: [p.WebGPUSystem], name: 'encoder', priority: 1 };
class j {
  constructor(e) {
    this._renderer = e;
  }
...
j.extension = { type: [p.WebGPUSystem], name: 'limits' };
class K {
  constructor(e) {
  ...
  onRenderTargetChange(e) {
    ...
    (t ||
      (t = this._renderTargetStencilState[e.uid] =
        { stencilMode: C.DISABLED, stencilReference: 0 }),
...
K.extension = { type: [p.WebGPUSystem], name: 'stencil' };
const G = {
  i32: { align: 4, size: 4 },
  u32: { align: 4, size: 4 },
  f32: { align: 4, size: 4 },
...
function Ue(a) {
  const e = a.map((r) => ({ data: r, offset: 0, size: 0 }));
  let t = 0;
  for (let r = 0; r < e.length; r++) {
    ...
    (s.data.size > 1 && (i = Math.max(i, n) * s.data.size),
      (t = Math.ceil(t / n) * n),
      (s.size = i),
      (s.offset = t),
      (t += i));
...
function Me(a, e) {
  const { size: t, align: r } = G[a.data.type],
    s = (r - t) / 4,
    i = a.data.type.indexOf('i32') >= 0 ? 'dataInt32' : 'data';
  return `
         v = uv.${a.data.name};
         ${e !== 0 ? `offset += ${e};` : ''}
...
function we(a) {
  return _e(a, 'uboWgsl', Me, be);
}
class q extends xe {
  constructor() {
    super({ createUboElements: Ue, generateUboSync: we });
  }
}
q.extension = { type: [p.WebGPUSystem], name: 'ubo' };
const b = 128;
class Y {
  constructor(e) {
    ((this._bindGroupHash = Object.create(null)),
      ...
      (this._batchBuffer = new ve({ minUniformOffsetAlignment: b })));
    const t = 256 / b;
    for (let r = 0; r < t; r++) {
      let s = T.UNIFORM | T.COPY_DST;
      (r === 0 && (s |= T.COPY_SRC),
        this._buffers.push(new he({ data: this._batchBuffer.data, usage: s })));
    }
  }
  renderEnd() {
  ...
  _getBufferResource(e) {
    if (!this._bufferResources[e]) {
      ...
      this._bufferResources[e] = new ye({ buffer: t, offset: ((e / 2) | 0) * 256, size: b });
    }
    return this._bufferResources[e];
  }
  _getBindGroup(e) {
    if (!this._bindGroups[e]) {
      const t = new A({ 0: this._getBufferResource(e) });
...
Y.extension = { type: [p.WebGPUPipes], name: 'uniformBatch' };
const Re = {
  'point-list': 0,
  'line-list': 1,
  'line-strip': 2,
...
class $ {
  ...
  setStencilMode(e) {
    this._stencilMode !== e &&
      ((this._stencilMode = e), (this._stencilState = Ge[e]), this._updatePipeHash());
  }
  setPipeline(e, t, r, s) {
  ...
  getPipeline(e, t, r, s) {
    ...
    const i = Ee(e._layoutKey, t._layoutKey, r.data, r._blendModeId, Re[s]);
    return this._pipeCache[i]
      ? this._pipeCache[i]
      : ((this._pipeCache[i] = this._createPipeline(e, t, r, s)), this._pipeCache[i]);
  }
    ...
    return (
      e.buffers.forEach((i) => {
        ...
        for (const u in t.attributeData) {
          ...
          ((c.divisor ?? 1) !== 1 &&
            v(
              `Attribute ${u} has an invalid divisor value of '${c.divisor}'. WebGPU only supports a divisor value of 1`,
            ),
            c.buffer === i &&
              ((n.arrayStride = c.stride),
              (n.stepMode = c.instance ? 'instance' : 'vertex'),
              o.push({
...
$.extension = { type: [p.WebGPUSystem], name: 'pipeline' };
class Ae {
  constructor() {
...
class De {
  ...
  getDescriptor(e, t, r) {
    ...
    const s = this._renderTargetSystem,
      ...
      n = e.colorTextures.map((c, l) => {
        const h = i.contexts[l];
        let f, d;
        (h
          ? (f = h.getCurrentTexture().createView())
          : (f = this._renderer.texture.getGpuSource(c).createView({ mipLevelCount: 1 })),
          i.msaaTextures[l] &&
            ((d = f), (f = this._renderer.texture.getTextureView(i.msaaTextures[l]))));
        const _ = t & x.COLOR ? 'clear' : 'load';
        return (
          r ?? (r = s.defaultClearColor),
          { view: f, resolveTarget: d, clearValue: r, storeOp: 'store', loadOp: _ }
        );
      });
    let o;
    if (
      ...
      const c = t & x.STENCIL ? 'clear' : 'load',
        l = t & x.DEPTH ? 'clear' : 'load';
      o = {
  ...
  clear(e, t = !0, r, s) {
    ...
    if (n.commandEncoder === null) {
      const c = o.createCommandEncoder(),
        l = this.getDescriptor(e, t, r),
        h = c.beginRenderPass(l);
    ...
    return (
      e.colorTextures.forEach((r, s) => {
        ...
        if (((t.msaa = r.source.antialias), r.source.antialias)) {
          const i = new ue({ width: 0, height: 0, sampleCount: 4 });
          t.msaaTextures[s] = i;
        }
      }),
      t.msaa &&
  ...
  resizeGpuRenderTarget(e) {
    ...
    ((t.width = e.width),
      ...
      t.msaa &&
        e.colorTextures.forEach((r, s) => {
          t.msaaTextures[s]?.resize(r.source.width, r.source.height, r.source._resolution);
        }));
  }
}
class X extends Se {
...
X.extension = { type: [p.WebGPUSystem], name: 'renderTarget' };
class Z {
  constructor() {
  ...
  _createGPUProgramData(e) {
    ...
    return (
      (this._gpuProgramData[e._layoutKey] = { bindGroups: r, pipeline: t.createPipelineLayout(s) }),
...
Z.extension = { type: [p.WebGPUSystem], name: 'shader' };
const g = {};
g.normal = {
...
class J {
  ...
  getColorTargets(e) {
    return [{ format: 'bgra8unorm', writeMask: 0, blend: g[e.blendMode] || g.normal }];
  }
  destroy() {
    this.gpu = null;
  }
}
J.extension = { type: [p.WebGPUSystem], name: 'state' };
const ke = {
    type: 'image',
    upload(a, e, t) {
      const r = a.resource,
      ...
      t.device.queue.writeTexture(
        { texture: e },
        r,
        { offset: 0, rowsPerImage: a.pixelHeight, bytesPerRow: a.pixelHeight * i },
  ...
  He = { blockBytes: 4, blockWidth: 1, blockHeight: 1 },
  ze = {
    type: 'compressed',
    upload(a, e, t) {
      let r = a.pixelWidth,
    ...
    upload(a, e, t) {
      ...
      if (globalThis.HTMLImageElement && r instanceof HTMLImageElement) {
        ...
        (o.getContext('2d').drawImage(r, 0, 0, r.width, r.height),
          (a.resource = o),
          v('ImageSource: Image element passed, converting to canvas and replacing resource.'));
      }
  ...
  generateMipmap(e) {
    ...
    if (!i) {
      ...
      for (let c = 1; c < e.mipLevelCount; ++c)
        (n.copyTextureToTexture({ texture: r, mipLevel: c - 1 }, { texture: e, mipLevel: c }, u),
...
class te {
  constructor(e) {
    ((this.managedTextures = []),
      ...
      (this._uploads = { image: ee, buffer: ke, video: Fe, compressed: ze }),
      (this._renderer = e),
  ...
  _initSource(e) {
    ...
    const r = Q[e.format] || { blockWidth: 1, blockHeight: 1 },
  ...
  _createTextureBindGroup(e) {
    const t = e.source;
    return (
      (this._bindGroupHash[e.uid] = new A({
        0: t,
        1: t.style,
        2: new D({ uTextureMatrix: { type: 'mat3x3<f32>', value: e.textureMatrix.mapCoord } }),
      })),
      this._bindGroupHash[e.uid]
    );
  }
  ...
  getPixels(e) {
    const t = this.generateCanvas(e),
      r = R.getOptimalCanvasAndContext(t.width, t.height),
      s = r.context;
    s.drawImage(t, 0, 0);
...
te.extension = { type: [p.WebGPUSystem], name: 'texture' };
class re {
  constructor() {
    this._maxTextures = 0;
  }
  contextChange(e) {
    const t = new D({
      uTransformMatrix: { value: new E(), type: 'mat3x3<f32>' },
      uColor: { value: new Float32Array([1, 1, 1, 1]), type: 'vec4<f32>' },
      uRound: { value: 0, type: 'f32' },
    });
    this._maxTextures = e.limits.maxBatchableTextures;
    const r = k({ name: 'graphics', bits: [pe, le(this._maxTextures), ge, H] });
    this.shader = new z({ gpuProgram: r, resources: { localUniforms: t } });
  }
  execute(e, t) {
    const r = t.context,
    ...
    for (let _ = 0; _ < u.instructionSize; _++) {
      ...
      if (
        (m.topology !== d &&
          ((d = m.topology),
          c.setPipelineFromGeometryProgramAndState(o.geometry, s.gpuProgram, e.state, m.topology)),
        (s.groups[1] = m.bindGroup),
        !m.gpuBindGroup)
      ) {
        const P = m.textures;
        ((m.bindGroup = L(P.textures, P.count, this._maxTextures)),
          (m.gpuBindGroup = i.bindGroup.getBindGroup(m.bindGroup, s.gpuProgram, 1)));
      }
      (c.setBindGroup(1, m.bindGroup, s.gpuProgram),
        c.renderPassEncoder.drawIndexed(m.size, 1, m.start));
    }
  }
  destroy() {
...
re.extension = { type: [p.WebGPUPipesAdaptor], name: 'graphics' };
class se {
  init() {
    const e = k({ name: 'mesh', bits: [me, Pe, H] });
    this._shader = new z({
      gpuProgram: e,
      resources: {
        uTexture: M.EMPTY._source,
        uSampler: M.EMPTY._source.style,
        textureUniforms: { uTextureMatrix: { type: 'mat3x3<f32>', value: new E() } },
      },
    });
  }
  execute(e, t) {
...
se.extension = { type: [p.WebGPUPipesAdaptor], name: 'mesh' };
const Ie = [...Te, q, N, U, j, W, te, X, Z, J, $, V, K, I],
  We = [...Ce, Y],
  Ve = [O, se, re],
  ie = [],
  ne = [],
