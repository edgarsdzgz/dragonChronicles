<script lang="ts">
  ...
  import { createAnimatedDragonSprite } from '$lib/pixi/dragon-sprites';
  import type { DragonAnimator } from '$lib/pixi/dragon-animator';
  import { createAnimatedEnemySprite, type EnemyType } from '$lib/pixi/enemy-sprites';
  ...
  import {
    ...
    type Projectile,
  } from '$lib/pixi/projectile-sprites';
  ...
  function drawAttackRange() {
    ...
    dragons.forEach((dragon) => {
      ...
      // Add range text
      const _rangeText = `${DRAGON_ATTACK_RANGE}px`;
  ...
  function flashEnemyDamage(enemy: (typeof enemies)[0]) {
    ...
    // Flash red and slightly enlarge
    enemy.sprite.tint = 0xff3333; // Bright red
    enemy.sprite.scale.set(originalScale * 1.1); // 10% larger for impact

    // Force render to show the flash immediately
    if (app) {
      app.renderer.render(app.stage);
    }

    // Restore original properties after flash duration
    setTimeout(() => {
      if (enemy.sprite) {
        enemy.sprite.tint = originalTint;
        enemy.sprite.scale.set(originalScale);

        // Force render to show the restoration
        if (app) {
          app.renderer.render(app.stage);
        }
      }
    }, 120); // 120ms flash duration for snappy feedback

    console.log(
      `${enemy.type} flashed red from damage (${enemy.health}/${enemy.maxHealth} HP remaining)`,
    );
  }

  function flashDragonDamage(dragon: (typeof dragons)[0]) {
    if (!dragon || !dragon.sprite) return;

    // Store original properties
    const originalTint = dragon.sprite.tint;
    const originalScale = dragon.sprite.scale.x;

    // Flash red and slightly enlarge
    dragon.sprite.tint = 0xff6666; // Slightly lighter red for dragons
    dragon.sprite.scale.set(originalScale * 1.08); // Smaller scale increase for dragons
    ...
    setTimeout(() => {
      if (dragon.sprite) {
        dragon.sprite.tint = originalTint;
        dragon.sprite.scale.set(originalScale);
    ...
    }, 100); // Slightly shorter flash for dragons

    console.log(`Dragon flashed red from enemy damage`);
  }
  ...
  async function fireProjectileFromEnemy(enemyIndex: number) {
    ...
    try {
      ...
      // Create collision callback that checks if projectile hits any dragon
      const dragonCollisionCallback = (projectileSprite: Sprite) => {
        // Check collision with all dragons using sprite bounds
        for (let i = 0; i < dragons.length; i++) {
          ...
          const dragonBounds = dragon.sprite.getBounds();
          ...
          if (isColliding) {
            console.log(`Enemy projectile hit dragon!`);

            // Flash dragon red when hit
            flashDragonDamage(dragon);
      ...
      console.log(
        `${enemy.type} fired projectile toward dragon (distance: ${Math.round(closestDistance)}px)`,
      );
      ...
      const collisionCallback = (projectileSprite: Sprite) => {
        ...
        for (let i = 0; i < enemies.length; i++) {
          ...
          if (isColliding) {
            ...
            console.log(
              `Dragon projectile hit ${enemy.type} for ${DRAGON_BASE_DAMAGE} damage! Health: ${enemy.health}/${enemy.maxHealth}`,
            );
  ...
  function _startDragonAttacking() {
    ...
    const scheduleNextSpawn = () => {
      ...
      autoSpawnInterval = window.setTimeout(() => {
        ...
        if (enemies.length < AUTO_SPAWN_CONFIG.maxEnemies) {
          ...
          console.log(
            `Auto-spawned ${randomEnemyType} (${enemies.length + 1}/${AUTO_SPAWN_CONFIG.maxEnemies})`,
          );
        } else {
          console.log(
            `Max enemies reached (${AUTO_SPAWN_CONFIG.maxEnemies}) - waiting for some to be defeated`,
          );
        }
  ...
  function spawnCenterDragon() {
    ...
    createAnimatedDragonSprite(app.renderer, app.stage)
      .then(({ sprite, animator }: { sprite: Sprite; animator: DragonAnimator }) => {
  ...
  async function spawnEnemy(enemyType: EnemyType) {
    ...
    try {
      ...
      animator.setFrameChangeCallback((frame: any, speedMultiplier: number) => {
        ...
        console.log(
          `${enemyType} movement speed updated: ${speedMultiplier.toFixed(2)}x (frame: ${frame})`,
        );
      });
...
<div
  style="position:fixed; inset:0; background: linear-gradient(to bottom, 
...
>
  ...
  <div
    style="position:absolute; right:8px; top:8px; background:rgba(0,0,0,.9); color:#fff; padding:16px; border-radius:12px; font:12px system-ui; min-width:280px; max-width:320px; backdrop-filter: blur(10px); overflow-wrap:break-word;"
  >
    <h3 style="margin:0 0 12px 0; font-size:16px; color:#4CAF50;">🐉 Dragon Animation Test</h3>

    <div
      style="margin-bottom:12px; padding:8px; background:rgba(255,255,255,.1); border-radius:6px;"
    >
    ...
    <div style="display:flex; flex-direction:column; gap:8px;">
      ...
      <button
        ...
        style="padding:8px 12px; background:{globalAnimationState
          ? '#FF9800'
          : '#4CAF50'}; color:white; border:none; border-radius:6px; cursor:pointer; font-size:11px;"
      >
      ...
      <div style="display:flex; gap:6px; margin-bottom:6px;">
        <button
          ...
          style="flex:1; padding:6px 8px; background:{showAttackRange
            ? '#FF9800'
            : '#607D8B'}; color:white; border:none; border-radius:4px; cursor:pointer; font-size:9px;"
        >
        ...
        <button
          ...
          style="flex:1; padding:6px 8px; background:{showEnemyAttackRange
            ? '#FF5722'
            : '#795548'}; color:white; border:none; border-radius:4px; cursor:pointer; font-size:9px;"
        >
      ...
      <div
        style="padding:8px; background:rgba(76, 175, 80, 0.2); border-radius:6px; text-align:center; margin-bottom:8px;"
      >
      ...
      <button
        ...
        disabled={!isLoaded || dragons.length === 0}
        style="width:100%; padding:12px; background:{autoSpawning
          ? '#FF5722'
          : '#2196F3'}; color:white; border:none; border-radius:8px; cursor:pointer; font-size:12px; font-weight:bold; margin-bottom:12px;"
      >
      ...
      {#if autoSpawning}
        <div
          style="padding:6px 8px; background:rgba(255, 87, 34, 0.2); border-radius:4px; text-align:center; margin-bottom:8px;"
        >
    ...
    <div style="border-top:1px solid #444; padding-top:12px; margin-top:12px;">
      ...
      <div style="font-size:10px; color:#888;">
        Enemy Health: Mantair {ENEMY_HEALTH_CONFIG['mantair-corsair']}HP | Swarm {ENEMY_HEALTH_CONFIG[
          'swarm'
        ]}HP
      </div>
      <div style="font-size:10px; color:#888; margin-top:4px;">
        Formation: Column {enemyFormation.currentColumn + 1}, Position {enemyFormation.currentPosition +
          1}
      </div>
      <div style="font-size:10px; color:#888;">
        Combat: {automaticCombat ? 'Automatic 🤖' : 'Manual'} | Moving: {enemies.filter(
          (e) => e.isMoving,
        ).length}
      </div>
      <div style="font-size:10px; color:#888;">
        Speed Range: {enemies.length > 0
          ? `${Math.min(...enemies.map((e) => e.currentSpeedMultiplier)).toFixed(1)}x - ${Math.max(...enemies.map((e) => e.currentSpeedMultiplier)).toFixed(1)}x`
          : 'none'}
      </div>
      <div style="font-size:10px; color:#888;">
        Spawning: {autoSpawning
          ? `Auto 🎮 (${enemies.length}/${AUTO_SPAWN_CONFIG.maxEnemies})`
          : 'Manual'}
      </div>
      <div style="font-size:10px; color:#888;">
    ...
    <div style="border-top:1px solid #444; padding-top:12px; margin-top:12px;">
      <div style="font-size:10px; color:#888; line-height:1.4;">
        ...
        {currentFPS} FPS ({Math.round(1000 / currentFPS)}ms/frame)
      </div>
    </div>
  </div>
</div>
