...
<div style="position:fixed; inset:0; background: #0a0a0a;">
  ...
  <div
    style="position:absolute; left:8px; top:8px; background:rgba(0,0,0,.95); color:#fff; padding:16px; border-radius:8px; font:11px 'Courier New', monospace; max-width:500px; max-height:80vh; overflow-y:auto;"
  >
    ...
    <div
      style="border:1px solid #333; padding:8px; background:#111; border-radius:4px; max-height:400px; overflow-y:auto;"
    >
      <div style="font-size:10px; color:#888; margin-bottom:6px;">Debug Log:</div>
      {#each debugInfo as message, _i (message)}
        <div
          style="margin:2px 0; color: {message.includes('ERROR')
            ? '#f44336'
