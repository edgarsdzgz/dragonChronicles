#!/usr/bin/env python3
"""
Improved script to find and remove ONLY the problematic dangling object literals
that are causing syntax errors, while preserving valid object structures.
"""
...
def find_problematic_dangling_objects(content):
    ...
    This version is more careful about object literals and preserves valid structures.
    """
    lines = content.split('\n')
    issues = []
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^\s*[a-zA-Z_][a-zA-Z0-9_]*\s*:\s*[^=]', stripped):
            ...
            # Special case: if this is part of a Text constructor or similar, it's legitimate
            if any(text_context in stripped for text_context in ['text:', 'style:', 'fontFamily:', 'fontSize:', 'fill:', 'fontWeight:', 'align:', 'stroke:', 'strokeThickness:']):
                context_found = True
...
def remove_dangling_objects(content, issues):
    ...
    This version is more careful about preserving object structures.
    """
    lines = content.split('\n')
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python cleanup-dangling-objects-v2.py <typescript-file>")
        sys.exit(1)
    ...
    print(f"Analyzing {file_path} for problematic dangling objects (v2)...")
