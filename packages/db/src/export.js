...
import { encodeExportV1, exportFileToBlob, blobToExportFile, generateChecksumSync, validateExportV1, } from './codec.js';
 ...
 */
export async function exportAllProfilesToBlob() {
    try {
 ...
 */
export async function importFromBlob(blob) {
    const result = {
        success: false,
        importedProfiles: 0,
        errors: [],
...
export async function validateExportBlob(blob) {
    ...
    try {
        ...
        result.details.profileIds = saveData.data.profiles.map((p) => p.id);
    }
    catch (error) {
 ...
 */
export async function getExportFileInfo(blob) {
    return {
        fileSize: blob.size,
        fileName: blob.name,
        mimeType: blob.type,
