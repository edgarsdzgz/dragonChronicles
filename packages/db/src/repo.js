 ...
 */
export async function getActiveSave(profileId) {
    try {
        ...
        const pointers = JSON.parse(pointer.value);
 ...
 */
export async function putSaveAtomic(profileId, saveData, keepBackups = 3, providedChecksum) {
    try {
 ...
 */
export async function getAllSaves(profileId) {
    try {
 ...
 */
async function _updateProfilePointer(profileId, saveId) {
    try {
 ...
 */
export async function getActiveSaveId(profileId) {
    try {
        const pointer = await db.meta.get(META_KEYS.PROFILE_POINTERS);
        if (!pointer)
            return null;
 ...
 */
async function _pruneBackups(profileId, keepCount) {
    try {
