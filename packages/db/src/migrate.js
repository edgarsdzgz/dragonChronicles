 ...
 */
async function runSingleMigration(version) {
    const startTime = Date.now();
    const result = {
        success: false,
        version,
    ...
    try {
        ...
        for (const save of saves) {
            try {
                ...
                result.errors.push(`Failed to migrate save ${save.id}: ${saveError instanceof Error ? saveError.message : 'Unknown error'}`);
            }
        }
        ...
        result.errors.push(`Migration ${version} failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
    result.timeMs = Math.max(1, Date.now() - startTime);
    return result;
}
    ...
    try {
        ...
        for (const save of saves) {
            try {
                ...
                result.issues.push(`Save ${save.id}: Validation error - ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
