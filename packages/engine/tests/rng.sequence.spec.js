...
const GOLDEN_U32_SEQUENCE = [
    0x8b5f4c3a, 0x2e7d9f1b, 0x4a6c8e2d, 0x1f3b5d7e, 0x9c2e4f6a,
    0x5d7e9f1b, 0x3c4e5f6a, 0x7d8e9f1a, 0x2b3c4d5e, 0x6f7e8d9c
];
const GOLDEN_FLOAT_SEQUENCE = [
    0.545, 0.182, 0.291, 0.122, 0.612,
    0.365, 0.236, 0.491, 0.169, 0.436
];
...
test('PCG32 produces deterministic float sequence', () => {
    ...
    for (let i = 0; i < GOLDEN_FLOAT_SEQUENCE.length; i++) {
        ...
        assert.ok(Math.abs(value - GOLDEN_FLOAT_SEQUENCE[i]) < 0.001, `Float sequence mismatch at index ${i}`);
    }
});
...
test('PCG32 advance works correctly', () => {
    ...
    assert.equal(rng1.nextU32(), rng2.nextU32(), 'Advance should produce same result as manual steps');
});
...
test('makeRngFromString produces deterministic results', () => {
    ...
    for (let i = 0; i < 10; i++) {
        assert.equal(rng1.nextU32(), rng2.nextU32(), `String seed should be deterministic at index ${i}`);
    }
});
test('RngStreams creates deterministic streams', () => {
    ...
    for (let i = 0; i < 10; i++) {
        assert.equal(spawns1.nextU32(), spawns2.nextU32(), `Spawns stream should be deterministic at index ${i}`);
    }
});
test('RngStreams different names produce different sequences', () => {
    ...
    assert.notEqual(spawnsValue, critsValue, 'Different stream names should produce different sequences');
});
...
test('RngStreams clone works correctly', () => {
    ...
    assert.equal(streams1.getStreamNames().length, streams2.getStreamNames().length, 'Clone should have same number of streams');
    // Streams should produce identical sequences
    const spawns1 = streams1.get('spawns');
    const spawns2 = streams2.get('spawns');
    for (let i = 0; i < 10; i++) {
        assert.equal(spawns1.nextU32(), spawns2.nextU32(), `Cloned streams should produce identical sequences at index ${i}`);
    }
});
await run();
