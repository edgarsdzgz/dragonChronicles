...
import { isUiToSim, isSimToUi, validateBuildVersion, validateAbilityCooldown, sanitizeSeed, validateOfflineTime, AbilityRateLimiter, ValidationContext } from '../src/shared/validation.js';
...
import { Lands, Wards, AbilityId } from '../src/shared/enums.js';
test('isUiToSim validates boot messages correctly', () => {
    ...
    assert.equal(isUiToSim({ t: MessageType.Boot, seed: 'invalid', build: 'test' }), false, 'Invalid seed should fail');
    assert.equal(isUiToSim({ t: MessageType.Boot, seed: 12345, build: '' }), false, 'Empty build should fail');
...
test('isUiToSim validates start messages correctly', () => {
    ...
    assert.equal(isUiToSim({ t: MessageType.Start, land: 999, ward: Wards.W1 }), false, 'Invalid land should fail');
    assert.equal(isUiToSim({ t: MessageType.Start, land: Lands.HorizonSteppe, ward: 999 }), false, 'Invalid ward should fail');
    assert.equal(isUiToSim({ t: MessageType.Start, land: Lands.HorizonSteppe }), false, 'Missing ward should fail');
});
...
test('isUiToSim validates ability messages correctly', () => {
    ...
    assert.equal(isUiToSim({ t: MessageType.Ability, id: 999 }), false, 'Invalid ability ID should fail');
    assert.equal(isUiToSim({ t: MessageType.Ability, id: 'invalid' }), false, 'Non-numeric ability ID should fail');
...
test('isUiToSim validates offline messages correctly', () => {
    ...
    assert.equal(isUiToSim({ t: MessageType.Offline, elapsedMs: -1 }), false, 'Negative elapsed time should fail');
    assert.equal(isUiToSim({ t: MessageType.Offline, elapsedMs: MAX_OFFLINE_MS + 1 }), false, 'Excessive elapsed time should fail');
    assert.equal(isUiToSim({ t: MessageType.Offline, elapsedMs: 'invalid' }), false, 'Non-numeric elapsed time should fail');
});
...
test('isSimToUi validates tick messages correctly', () => {
    // Valid tick message
    const validTick = { t: MessageType.Tick, now: 1000, stats: { fps: 60, enemies: 5, proj: 10, dps: 100 } };
    assert.equal(isSimToUi(validTick), true, 'Valid tick message should pass validation');
    // Invalid tick messages
    assert.equal(isSimToUi({ t: MessageType.Tick, now: 'invalid', stats: {} }), false, 'Invalid now time should fail');
    assert.equal(isSimToUi({ t: MessageType.Tick, now: 1000, stats: 'invalid' }), false, 'Invalid stats should fail');
    assert.equal(isSimToUi({ t: MessageType.Tick, now: 1000, stats: { fps: 'invalid', enemies: 5, proj: 10, dps: 100 } }), false, 'Invalid fps should fail');
});
test('isSimToUi validates log messages correctly', () => {
    ...
    assert.equal(isSimToUi({ t: MessageType.Log, lvl: 'invalid', msg: 'test' }), false, 'Invalid log level should fail');
    assert.equal(isSimToUi({ t: MessageType.Log, lvl: LogLvl.Info, msg: 123 }), false, 'Non-string message should fail');
    assert.equal(isSimToUi({ t: MessageType.Log, lvl: LogLvl.Info }), false, 'Missing message should fail');
});
test('isSimToUi validates fatal messages correctly', () => {
    ...
    assert.equal(isSimToUi({ t: MessageType.Fatal, reason: 123 }), false, 'Non-string reason should fail');
...
test('validateAbilityCooldown works correctly', () => {
    ...
    assert.equal(validateAbilityCooldown(0, currentTime), true, 'Should pass if enough time has passed');
    assert.equal(validateAbilityCooldown(currentTime - ABILITY_COOLDOWN_MS, currentTime), true, 'Should pass if exactly cooldown time has passed');
    // Should fail if not enough time has passed
    assert.equal(validateAbilityCooldown(currentTime - ABILITY_COOLDOWN_MS + 1, currentTime), false, 'Should fail if not enough time has passed');
    assert.equal(validateAbilityCooldown(currentTime, currentTime), false, 'Should fail if no time has passed');
});
...
test('validateOfflineTime works correctly', () => {
    ...
    assert.equal(validateOfflineTime(MAX_OFFLINE_MS + 1), false, 'Excessive offline time should be invalid');
...
test('AbilityRateLimiter works correctly', () => {
    ...
    assert.equal(limiter.canUseAbility(1000 + ABILITY_COOLDOWN_MS - 1), false, 'Second ability should fail within cooldown');
    // Second ability should work if cooldown has passed
    assert.equal(limiter.canUseAbility(1000 + ABILITY_COOLDOWN_MS), true, 'Second ability should work after cooldown');
    // Reset should work
    limiter.reset();
