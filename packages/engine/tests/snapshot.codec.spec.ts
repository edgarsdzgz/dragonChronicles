...
import {
  encodeSnapshot,
  decodeSnapshot,
  encodeSnapshotStream,
  decodeSnapshotStream,
  validateSnapshot,
  createSnapshot,
  snapshotsEqual,
  getSnapshotDiff,
...
} from '../src/sim/protocol/codec.js';
import {
  hash64,
  hashSnapshotStream,
  FastHasher,
  HashComparator,
} from '../src/sim/snapshot/hasher.js';
...
test('validateSnapshot works correctly', () => {
  ...
  assert.equal(
    validateSnapshot('not an object'),
    false,
    'Non-object snapshot should fail validation',
  );
  assert.equal(
    validateSnapshot({ now: 'invalid', enemies: 5, proj: 10, fps: 60 }),
    false,
    'Invalid now should fail validation',
  );
  assert.equal(
    validateSnapshot({ now: 1000, enemies: NaN, proj: 10, fps: 60 }),
    false,
    'NaN enemies should fail validation',
  );
  assert.equal(
    validateSnapshot({ now: 1000, enemies: 5, proj: 10, fps: Infinity }),
    false,
    'Infinity fps should fail validation',
  );
});
...
test('snapshotsEqual works correctly', () => {
  const snapshot1: Snapshot = { now: 1000, enemies: 5, proj: 10, fps: 60 };
  const snapshot2: Snapshot = { now: 1000, enemies: 5, proj: 10, fps: 60 };
  const snapshot3: Snapshot = { now: 1000, enemies: 5, proj: 10, fps: 61 };

  assert.equal(snapshotsEqual(snapshot1, snapshot2), true, 'Identical snapshots should be equal');
  assert.equal(
    snapshotsEqual(snapshot1, snapshot3),
    false,
...
test('getSnapshotDiff works correctly', () => {
  const snapshot1: Snapshot = { now: 1000, enemies: 5, proj: 10, fps: 60 };
  const snapshot2: Snapshot = { now: 2000, enemies: 8, proj: 15, fps: 58 };
...
test('formatSnapshot works correctly', () => {
  ...
  assert.equal(
    formatted,
    't=1000ms enemies=5 proj=10 fps=60',
    'Formatted snapshot should match expected format',
  );
});
