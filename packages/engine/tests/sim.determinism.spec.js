...
import { encodeSnapshotStream } from '../src/sim/protocol/codec.js';
...
test('simulation produces deterministic snapshots', () => {
    ...
    assert.deepEqual(run1.snapshots, run2.snapshots, 'Two runs with same seed should produce identical snapshots');
...
test('simulation produces different snapshots for different seeds', () => {
    ...
    assert.notDeepEqual(run1.snapshots, run2.snapshots, 'Different seeds should produce different snapshots');
});
test('simulation maintains timing precision', () => {
    ...
    for (let i = 1; i < run.snapshots.length; i++) {
        ...
        assert.ok(interval >= GOLDEN_SNAPSHOT_INTERVAL - 10, `Snapshot interval should be close to ${GOLDEN_SNAPSHOT_INTERVAL}ms, got ${interval}ms`);
        assert.ok(interval <= GOLDEN_SNAPSHOT_INTERVAL + 10, `Snapshot interval should be close to ${GOLDEN_SNAPSHOT_INTERVAL}ms, got ${interval}ms`);
    }
});
test('simulation produces expected number of snapshots', () => {
    ...
    assert.ok(run.snapshots.length >= expectedCount - 1, `Should have at least ${expectedCount - 1} snapshots`);
    assert.ok(run.snapshots.length <= expectedCount + 1, `Should have at most ${expectedCount + 1} snapshots`);
});
test('simulation maintains deterministic RNG sequences', () => {
    const run1 = createDeterministicRun(GOLDEN_SEED, GOLDEN_DURATION_MS);
    const run2 = createDeterministicRun(GOLDEN_SEED, GOLDEN_DURATION_MS);
    // Check that RNG sequences are identical
    assert.deepEqual(run1.rngSequence, run2.rngSequence, 'RNG sequences should be identical for same seed');
});
...
test('simulation handles edge cases deterministically', () => {
    ...
    for (const seed of edgeSeeds) {
        ...
        assert.equal(run1.hash, run2.hash, `Edge case seed ${seed} should produce deterministic results`);
    }
});
...
// Helper function to create a deterministic simulation run
function createDeterministicRun(seed, durationMs) {
    ...
    // Create fixed clock
    const clock = new FixedClock(step);
    // Start recording
...
function createDeterministicRunSync(seed, durationMs) {
    const streams = createStandardStreams(seed);
    const snapshotWriter = new SnapshotWriter(GOLDEN_SNAPSHOT_INTERVAL);
    const rngSequence = [];
    let currentTime = 0;
