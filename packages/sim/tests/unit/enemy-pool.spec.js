...
import { EnemyPool, DEFAULT_POOL_CONFIG } from '../../src/enemies/enemy-pool.js';
...
describe('EnemyPool', () => {
  ...
  describe('Allocation', () => {
    ...
    it('should generate unique IDs for each allocation', () => {
      const enemy1 = pool.allocate(1, { x: 0, y: 0 }, 1, 1, 0);
      const enemy2 = pool.allocate(2, { x: 0, y: 0 }, 1, 1, 0);
    ...
    it('should expand pool when needed', () => {
      ...
      const enemy = pool.allocate(1, { x: 0, y: 0 }, 1, 1, 0);
      expect(enemy).not.toBeNull();
    ...
    it('should return null when pool is at maximum size and full', () => {
      ...
      smallPool.allocate(1, { x: 0, y: 0 }, 1, 1, 0);
      // Next allocation should fail
      const enemy = smallPool.allocate(1, { x: 0, y: 0 }, 1, 1, 0);
  ...
  describe('Deallocation', () => {
    ...
    it('should reset enemy properties on deallocation', () => {
      const enemy = pool.allocate(1, { x: 100, y: 200 }, 1, 1, 50);
      pool.deallocate(enemy);
  ...
  describe('Statistics', () => {
    it('should track peak usage correctly', () => {
      ...
      const enemies = [];
      for (let i = 0; i < 5; i++) {
        enemies.push(pool.allocate(1, { x: 0, y: 0 }, 1, 1, 0));
      }
  ...
  describe('Edge Cases', () => {
    it('should handle allocation with zero initial size', () => {
      ...
      const enemy = zeroPool.allocate(1, { x: 0, y: 0 }, 1, 1, 0);
      expect(enemy).not.toBeNull();
