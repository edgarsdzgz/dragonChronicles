...
import { DEFAULT_SPAWN_CONFIG, createWardConfigs, calculateSpawnRate, getEnemyConfig, selectEnemyFamily, createSpawnConfig, validateSpawnConfig, } from '../../src/enemies/spawn-config.js';
describe('Spawn Configuration', () => {
    ...
    describe('Enemy Family Selection', () => {
        ...
        it('should select enemy family based on weights', () => {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                const family = selectEnemyFamily(config, 1, randomValue);
                if (family) {
        ...
        it('should return undefined for invalid ward', () => {
            const family = selectEnemyFamily(config, 999, 0.5);
            expect(family).toBeUndefined();
        });
        it('should handle edge case random values', () => {
            const family0 = selectEnemyFamily(config, 1, 0);
            const family1 = selectEnemyFamily(config, 1, 1);
    ...
    describe('Configuration Validation', () => {
        ...
        it('should detect ward with no enemy types', () => {
            const config = createSpawnConfig();
            const wardConfig = config.wardConfigs.get(1);
            if (wardConfig) {
                wardConfig.availableEnemyTypes = [];
            }
            const errors = validateSpawnConfig(config);
            expect(errors.some((error) => error.includes('must have at least one enemy type'))).toBe(true);
        });
        it('should detect ward with invalid spawn rate multiplier', () => {
            ...
            const errors = validateSpawnConfig(config);
            expect(errors.some((error) => error.includes('spawn rate multiplier must be positive'))).toBe(true);
        });
    });
    describe('Edge Cases', () => {
