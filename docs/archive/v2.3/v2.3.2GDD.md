...
1. [Parallax Background System](#5-parallax-background-system)
...
The parallax background system creates immersive depth through layered visual elements that move at different speeds, simulating the dragon's flight through various landscapes. Each land features unique background assets organized in a consistent layer structure.

### Layer Architecture (Land 1: Horizon Steppe)

#### Layer Stack (Back to Front)
1. **Static Background** - Base sky and underground areas
2. **Clouds Parallax** - Sky clouds behind mountain (12.5% speed)
3. **Mountain Parallax** - Distant mountain range (7.5% speed)
4. **Hills Parallax** - Mid-ground rolling hills (6% speed)
5. **Grassland Foreground** - Moving grass layer (100% speed)

#### Positioning Reference
- **Action Band Bottom Y**: ~400px at 1080p (~37% of screen height)
- **Clouds Base**: +0px from actionBandBottomY (aligned with action band bottom)
- **Mountain Base**: +32px from actionBandBottomY
- **Hills Base**: +11px from actionBandBottomY (21px higher than mountain - **LOCKED IN** for perfect visibility)
- **Grassland Top**: +44px from actionBandBottomY

#### Asset Dimensions (Land 1: Horizon Steppe)
- **Background**: `steppe_background_grassless.png` - 2048x1024px
- **Clouds**: `steppe_clouds-1.png` - Variable width x height (sky layer)
- **Mountain**: `lonelyMountain-clouds-1.png` - 850x425px
- **Hills**: `steppe_hills-1.png` - Variable width x height (similar to grassland)
...
```
backgrounds/
├── land1_horizon_steppe/
│   ├── background/
│   │   └── steppe_background_grassless.png
│   ├── parallax/
│   │   ├── mountains/
│   │   │   └── lonelyMountain-clouds-1.png
│   │   └── hills/
│   │       └── steppe_hills-1.png
│   └── foreground/
│       └── grasslandLayer_steppe.png
├── land2_ember_reaches/
│   └── [volcanic theme assets]
└── land3_mistral_peaks/
    └── [mountain/ice theme assets]
```
...
const BACKGROUND_ASSETS = {
  land1: {
    background: '/backgrounds/land1_horizon_steppe/background/steppe_background_grassless.png',
    parallax: {
      mountains: '/backgrounds/land1_horizon_steppe/parallax/mountains/lonelyMountain-clouds-1.png',
      hills: '/backgrounds/land1_horizon_steppe/parallax/hills/steppe_hills-1.png'
    },
    foreground: '/backgrounds/land1_horizon_steppe/foreground/grasslandLayer_steppe.png'
  }
  // Future lands...
};
```
...
#### Responsive Scaling
- All pixel offsets multiplied by `currentScale` for responsive positioning
- Reference measurements at 1080p, scale up/down for different screen sizes
- Maintains visual proportions across all device types

#### Performance Considerations
- Only load assets for current land
- Unload previous land assets when transitioning
- Use efficient sprite pooling for parallax elements
- Maintain 60fps on desktop, ≥40fps on mobile

### Quality Standards

#### Visual Requirements
- No gaps or overlaps between layers
- Seamless horizontal tiling for all parallax elements
- Consistent depth perception through speed relationships
- Proper z-index ordering prevents rendering conflicts
