...
1. Integration tests: `tsc -b packages/shared packages/logger packages/sim`

1. E2E tests: `tsc -b` (full workspace)
...
# Driver builds once, then runs all tests with BUILD_ONCE=1 (2)
...
1. **Read tiny-runner output**: Shows which specific tests failed

1. **Run individual tests**: Isolate failing test file

1. **Check assertions**: Review assertion error messages

### Build Failures

1. **TypeScript errors**: Check `tsc -b` output for compilation errors

1. **Missing dependencies**: Ensure `pnpm install` completed successfully

1. **Path resolution**: Verify `require.resolve("typescript/bin/tsc")` works
